...
function createClient(xhrInstanceId) {

  // If got a number, we assume it is the xhrInstanceId, otherwise, we assume it is the XHR and
  // extract the xhrInstanceId from it - we don't want to hold a reference to the XHR in a variable
  // to avoid indirect memory leaks through the context
  if (typeof(xhrInstanceId) !== "number") {
    xhrInstanceId = xhrInstanceId.__xhrInstanceId;
  }
  ...
  const getXhrFlag = () => {
    return getXHR()[xhrSymbols.flag];
  }

  const getXhrProperties = () => {
    return getXHR()[xhrSymbols.properties];
  }
  
  const urlObj = new URL(getXhrFlag().uri);
  const uri = urlObj.href;
  const ucMethod = getXhrFlag().method.toUpperCase();

  const { requestManager } = getXhrFlag();
  ...
  if (urlObj.protocol === "file:") {
    ...
    if (requestManager) {
      const req = {
        abort() {
          try {
            getXhrProperties().abortError = true;
            getXHR().abort();
          } catch (err) {
            console.warn('XHR failed to handle abort request: %o', err);
          }
        }
      };
  ...
  /////////////////////////////////////////////////////////
  // This section is not relevant to handling memory leaks.
  // This adds support for fetching blobs (from a _blobManager 
  // attached to the Window) through the XHR. Not sure this is
  // really needed.
  if (urlObj.protocol === "blob:") {
    ...
    let bufferPromise;
    try {
      bufferPromise = getXHR()._ownerDocument.defaultView._blobManager.getObjectAsBufferAsync(uri, getXHR()._ownerDocument.defaultView.Blob, getXHR()._ownerDocument.defaultView.FileReader);
      if (!bufferPromise) {
        throw new Error(`Could not find object with Url: ${uri}`);
      }
    } catch (err) {
      process.nextTick(() => client.emit("error", err));
      return client;
    ...
    bufferPromise.then((buffer) => {
      ...
      process.nextTick(() => {
        try {
          getXhrProperties().responseBuffer = buffer;
          getXhrProperties().totalReceivedChunkSize = buffer.length;
          response.emit("end");
          client.emit("end");
        } catch (err) {
          process.nextTick(() => client.emit("error", err));
  ...
  for (const header in getXhrFlag().requestHeaders) {
    requestHeaders[header] = getXhrFlag().requestHeaders[header];
  }

  if (getRequestHeader(getXhrFlag().requestHeaders, "referer") === null) {
    requestHeaders.Referer = getXhrFlag().referrer;
  }
  if (getRequestHeader(getXhrFlag().requestHeaders, "user-agent") === null) {
    requestHeaders["User-Agent"] = getXhrFlag().userAgent;
  }
  if (getRequestHeader(getXhrFlag().requestHeaders, "accept-language") === null) {
    requestHeaders["Accept-Language"] = "en";
  }
  if (getRequestHeader(getXhrFlag().requestHeaders, "accept") === null) {
    requestHeaders.Accept = "*/*";
  }

  const crossOrigin = getXhrFlag().origin !== urlObj.origin;
  if (crossOrigin) {
    requestHeaders.Origin = getXhrFlag().origin;
  }

  const options = {
    uri,
    method: getXhrFlag().method,
    headers: requestHeaders,
    gzip: true,
    maxRedirects: 21,
    followAllRedirects: true,
    encoding: null,
    strictSSL: getXhrFlag().strictSSL,
    proxy: getXhrFlag().proxy,
    forever: true
  };
  if (getXhrFlag().auth) {
    options.auth = {
      user: getXhrFlag().auth.user || "",
      pass: getXhrFlag().auth.pass || "",
      sendImmediately: false
    };
  }
  if (getXhrFlag().cookieJar && (!crossOrigin || getXhrFlag().withCredentials)) {
    options.jar = wrapCookieJarForRequest(getXhrFlag().cookieJar);
  }

  const { body } = getXhrFlag();
  const hasBody = body !== undefined &&
                  body !== null &&
                  body !== "" &&
                  !(ucMethod === "HEAD" || ucMethod === "GET");

  if (hasBody && !getXhrFlag().formData) {
    options.body = body;
  }

  if (hasBody && getRequestHeader(getXhrFlag().requestHeaders, "content-type") === null) {
  ...
  const nonSimpleHeaders = Object.keys(getXhrFlag().requestHeaders)
    .filter(header => !simpleHeaders.has(header.toLowerCase()));

  if (crossOrigin && (!simpleMethods.has(ucMethod) || nonSimpleHeaders.length > 0 || getXhrProperties().uploadListener)) {
    ...
    preflightRequestHeaders["Access-Control-Request-Method"] = getXhrFlag().method;
    if (nonSimpleHeaders.length > 0) {
      preflightRequestHeaders["Access-Control-Request-Headers"] = nonSimpleHeaders.join(", ");
    }

    preflightRequestHeaders["User-Agent"] = getXhrFlag().userAgent;

    getXhrFlag().preflight = true;
    ...
    preflightClient.on("response", resp => {
      try {
        ...
        if (!validCORSPreflightHeaders(getXHR(), resp, flag, properties)) {
          setResponseToNetworkError(getXHR());
          return;
        }
        ...
        console.warn('XHR failed to handle preflight response: %o', err);
      }
    });
