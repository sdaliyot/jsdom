...
// Similar to how we handle the Window object, we also avoid holding a reference to the XHR in a variable
// to avoid indirect memory leaks through the context
const getXHR = (xhrInstanceId) => {
  return global.__JSDOM_XHRS[xhrInstanceId];
}

module.exports = function createXMLHttpRequest(windowInstanceId) {

  // If got a number, we assume it is the windowInstanceId, otherwise, we assume it is the Window and
  // extract the windowInstanceId from it - we don't want to hold a reference to the Window in a variable
  // to avoid indirect memory leaks through the context
  if (typeof(windowInstanceId) !== "number") {
    windowInstanceId = windowInstanceId.__windowInstanceId;
  }

  const getWindow = () => {
    return global.__JSDOM_WINDOWS[windowInstanceId];
  }
  ...
  const getProgressEvent = () => {
    return getWindow().ProgressEvent;
  }

  const emitLog = (logFuncName, msg, ...args) => {
    try {
      getWindow().document.defaultView._virtualConsole.emit(logFuncName, msg, ...args);
    } catch (err) {
      console.error('Failed to emit log: %o', err);
    }
  }

  const removeXHR = (xhrInstanceId) => {

    emitLog("log", 'Removing XHR %o', xhrInstanceId);
  
    delete global.__JSDOM_XHRS[xhrInstanceId];
    delete global.__JSDOM_XHRS_INFO[xhrInstanceId];
  }
  
  const startLeakingXHRsMonitor = () => {
    setTimeout(() => {
      const now = Date.now();
      ...
      for (const xhrInstanceId of xhrInfoKeys) {
        ...
        if (now - xhrInfo.createdTime > 300000) { // 5 min
          console.warn('XHR created more than 5 minutes ago and still not removed %o: %o', xhrInstanceId, global.__JSDOM_XHRS[xhrInstanceId]);
          try {
            // Look for an active window to emit the log to
            const windowKeys = Object.keys(global.__JSDOM_WINDOWS);
            global.__JSDOM_WINDOWS[windowKeys[0]].document.defaultView._virtualConsole.emit("warn", 'XHR created more than 5 minutes ago and still not removed %o: %o', xhrInstanceId, global.__JSDOM_XHRS[xhrInstanceId]);
          } catch (err) {
            console.error('Failed to emit warn: %o', err);
          }
        }
      }
    }, 300000);
  ...
  class XMLHttpRequest extends XMLHttpRequestEventTarget.interface {
    constructor() { // eslint-disable-line constructor-super
      ...
      // Store the XHR in global.__JSDOM_XHRS so we can access it without holding a direct reference to it because
      // such a reference causes memory leaks since the XHR is indirectly referenced through the context
      if (!global.__JSDOM_XHRS) {
        global.__JSDOM_XHRS = {};
        global.__JSDOM_XHRS_INFO = {};
        global.__JSDOM_XHR_NEXT_INSTANCE_ID = 1;
        startLeakingXHRsMonitor();
      }
      const xhrInstanceId = (global.__JSDOM_XHR_NEXT_INSTANCE_ID++).toString();
      global.__JSDOM_XHRS[xhrInstanceId] = theThis;
      global.__JSDOM_XHRS_INFO[xhrInstanceId] = {
        windowInstanceId: windowInstanceId,
        createdTime: Date.now(),
      }
      theThis.__xhrInstanceId = xhrInstanceId;

      emitLog("log", 'Creating XHR %o under window %o', xhrInstanceId, windowInstanceId);
    
      theThis.upload = XMLHttpRequestUpload.create();
      // We "override" the _ownerDocument property to return the Document through the windowInstanceId,
      // otherwise, the Document is causing indirect references to the Window object through the context
      const desc_upload_ownerDocument = {
        configurable: true,
        enumerable: true,
        set: function(v) {
          // If got a number, we assume it is the windowInstanceId, otherwise, we assume it is the Document and
          // extract the windowInstanceId from it
          if (typeof(v) !== "number") {
            v = v.defaultView.__windowInstanceId;
          }
          this.__windowInstanceId = v;
        },
        get: function() {
          const docWindowInstanceId = this.__windowInstanceId;
          if (docWindowInstanceId) {
            return global.__JSDOM_WINDOWS[docWindowInstanceId].document;
          }
          return undefined;
        }
      };
      Object.defineProperty(theThis.upload, '_ownerDocument', desc_upload_ownerDocument);
      theThis.upload._ownerDocument = getWindow().document;
      ...
      theThis[xhrSymbols.flag] = {
        ...
        strictSSL: getWindow()._resourceLoader._strictSSL,
        proxy: getWindow()._resourceLoader._proxy,
    ...
    abort() {
      ...
      emitLog("log", 'Aborting XHR %o', this.__xhrInstanceId);
      ...
      removeXHR(this.__xhrInstanceId);
    ...
    open(method, uri, asynchronous, user, password) {
      ...
      emitLog("log", 'Opening XHR %o with %o', this.__xhrInstanceId, { method, uri, asynchronous, user, password });
    ...
    send(body) {
      ...
      emitLog("log", 'Sending XHR %o', this.__xhrInstanceId);
      ...
      if (flag.synchronous) {
        ...
        emitLog("log", 'Got synchronous XHR %o', this.__xhrInstanceId);
        ...
        if (response.properties.error) {
          ...
          const ProgressEvent = getProgressEvent();
        ...
        emitLog("log", 'Got asynchronous XHR %o', this.__xhrInstanceId);
        ...
        // Note that although we pass a reference to the XHR object, internally in xhrUtils we only keep 
        // the xhrInstanceId in a variable
        ...
        // Moved all the listeners to separate functions to avoid indirect references 
        // to Window/XHR objects through the context
        client.on("error", getClientErrorListener(this.__xhrInstanceId));

        client.on("response", getClientResponseListener(this.__xhrInstanceId));

        client.on("redirect", getClientRedirectListener(this.__xhrInstanceId));
    ...
    get _ownerDocument() {
      ...
      return document && idlUtils.implForWrapper(document);
    }
  }
  ...
  const getClientRedirectListener = (xhrInstanceId) => {
    return () => {
      try {
        ...
        emitLog("warn", 'XHR %o failed to handle client redirect event: %o', xhrInstanceId, err);
      }
    };
  };

  const getClientResponseListener = (xhrInstanceId) => {
    return (response) => {
      try {
        const xhr = getXHR(xhrInstanceId);
        const properties = xhr[xhrSymbols.properties];
        const flag = xhr[xhrSymbols.flag];

        const { statusCode } = response;

        properties.byteOffset = 0;
        ...
        properties.progressObj = { lengthComputable: false };
        // let lastProgressReported;
        if (bufferLength !== 0) {
          properties.progressObj.total = bufferLength;
          properties.progressObj.loaded = 0;
          properties.progressObj.lengthComputable = true;
        }
        // pre-allocate buffer.
        ...
        // Content-Length).
        response.on("data", getResponseDataListener(xhr.__xhrInstanceId));

        properties.client.on("data", getClientDataListener(xhr.__xhrInstanceId));
        properties.client.on("end", getClientEndListener(xhr.__xhrInstanceId));
      } catch (err) {
        emitLog("warn", 'XHR %o failed to handle client response event: %o', xhrInstanceId, err);
      }
    };
  };

  const getResponseDataListener = (xhrInstanceId) => {
    return (chunk) => {
      try {
        const xhr = getXHR(xhrInstanceId);
        const properties = xhr[xhrSymbols.properties];
        properties.byteOffset += chunk.length;
        properties.progressObj.loaded = properties.byteOffset;
      } catch (err) {
        emitLog("warn", 'XHR %o failed to handle response data event: %o', xhrInstanceId, err);
      }
    };
  };

  const getClientDataListener = (xhrInstanceId) => {
    return (chunk) => {
      try {
        const xhr = getXHR(xhrInstanceId);
        const properties = xhr[xhrSymbols.properties];
        ...
        if (properties.progressObj.total !== properties.progressObj.loaded || properties.totalReceivedChunkSize === properties.byteOffset) {
          if (properties.lastProgressReported !== properties.progressObj.loaded) {
            ...
            properties.lastProgressReported = properties.progressObj.loaded;
            const ProgressEvent = getProgressEvent();
            xhr.dispatchEvent(new ProgressEvent("progress", properties.progressObj));
          }
        }
      } catch (err) {
        emitLog("log", 'XHR %o failed to handle client data event: %o', xhrInstanceId, err);
      }
    };
  };

  const getClientEndListener = (xhrInstanceId) => {
    return () => {
      try {

        emitLog("log", 'On client end listener XHR %o', xhrInstanceId);
        ...
        properties.timeoutStart = 0;
        properties.client = null;
        removeXHR(xhrInstanceId);
        const ProgressEvent = getProgressEvent();
        xhr.dispatchEvent(new ProgressEvent("progress", properties.progressObj));
        readyStateChange(xhr, XMLHttpRequest.DONE);
        xhr.dispatchEvent(new ProgressEvent("load", properties.progressObj));
        xhr.dispatchEvent(new ProgressEvent("loadend", properties.progressObj));
      } catch (err) {
        emitLog("warn", 'XHR %o failed to handle client end event: %o', xhrInstanceId, err);
      }
    };
  };

  const getClientErrorListener = (xhrInstanceId) => {
    return (err) => {
      try {

        emitLog("log", 'On error XHR %o', xhrInstanceId);

        const xhr = getXHR(xhrInstanceId);
        const properties = xhr[xhrSymbols.properties];
        removeXHR(xhr.__xhrInstanceId);
        properties.error = err;
        xhrUtils.dispatchError(xhr);
      } catch (err) {
        emitLog("warn", 'XHR %o failed to handle client error event: %o', xhrInstanceId, err);
      }
    };
  };

  const getClientRequestListener = (xhrInstanceId, lengthComputable, total) => {
    return (req) => {
      req.on("response", () => {
        try {

          emitLog("log", 'On client request listener XHR %o', xhrInstanceId);
          ...
          emitLog("warn", 'XHR %o failed to handle client request response event: %o', xhrInstanceId, err);
        }
      });
    };
  };
  ...
  function setDispatchProgressEvents(xhr) {
    ...
    client.on("request", getClientRequestListener(xhr.__xhrInstanceId, lengthComputable, total));
  }
